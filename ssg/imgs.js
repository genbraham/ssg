...
export async function processImage(absPath, relPath, distDir) {
	...
	for (const [label, { width, quality }] of Object.entries(sizes)) {
		// resize input to target width, grayscale for clean channel math
		...
		// create solid main-color canvas (same size as resized image)
		const blueBg = sharp({
			create: {
				width: w,
				height: h,
		...
		// omposite black lines on top of blue
